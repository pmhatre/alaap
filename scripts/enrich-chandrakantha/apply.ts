/**
 * Apply Chandrakantha enrichment plans to Neo4j.
...
/** Update raga properties and add "chandrakantha" to sources */
async function updateRagaProperties(plan: EnrichmentPlan): Promise<void> {
  ...
  // Always update sources to include "chandrakantha"
  setClauses.push(
    `r.sources = CASE
      WHEN r.sources IS NULL THEN ["chandrakantha"]
      WHEN NOT "chandrakantha" IN r.sources THEN r.sources + "chandrakantha"
      ELSE r.sources END`,
  );
