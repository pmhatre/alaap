/**
 * Apply Wikipedia enrichment plans to Neo4j.
...
/** Update raga properties and add "wikipedia" to sources */
async function updateRagaProperties(plan: EnrichmentPlan): Promise<void> {
  ...
  // Always update sources to include "wikipedia"
  setClauses.push(
    `r.sources = CASE
      WHEN r.sources IS NULL THEN ["wikipedia"]
      WHEN NOT "wikipedia" IN r.sources THEN r.sources + "wikipedia"
      ELSE r.sources END`,
  );
